...
const ArticleManagerStudent = () => {
    ...
    const { inputFileRef, inputImageRef } = useRef(null);
        ...
        if (file) {
            ...
            if (validFormats.includes(file.type)) {
              ...
              if (type !== "file") {
                notification.error({
                    message: "Error",
                    description: "Please select a valid doc file .doc.",
                });
                setFileFormatError(true);

                setImage(null);
                inputFileRef.current.value = null;// Hiển thị thông báo nếu file không đúng định dạng
                if (type !== "image") {
                    notification.error({
                        message: "Error",
                        description:
