...
const DashBoard = () => {
    ...
    const [articleByAcademicYear, setArticleByAcademicYear] = useState([])
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const articlesByAcademic = await Promise.resolve(dashBoardApi.getTotalArticlesWithAcademicYear);

                const articlesData = articlesByAcademic.data.map((item) => ({
                    name: item.name,
                    count: item.count,
                }))

                setArticleByAcademicYear(articlesData)
                const privateData = [
