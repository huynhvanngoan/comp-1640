...
const AcademicYear = () => {
    ...
    const handleOkUser = async (values) => {
        setLoading(true);
        try {
            const closureDate = values.closureDate;
            const finalClosureDate = values.finalClosureDate;

            // Kiểm tra ngày finalClosureDate phải sau closureDate
            if (values.finalClosureDate && moment(values.finalClosureDate).isAfter(values.closureDate)) {
                notification.error({
