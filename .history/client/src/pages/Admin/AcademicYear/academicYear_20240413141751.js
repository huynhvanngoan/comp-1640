    ...
    const handleEditAcademic = (id) => {
        ...
        (async () => {
            try {
                ...
                setId(id);
                const closureDate = moment(response.data.closureDate).format(
                    "YYYY-MM-DD"
                );
                const finalClosureDate = moment(
                    response.data.finalClosureDate
                ).format("YYYY-MM-DD");
                form2.setFieldsValue({
                    name: response.data.name,
                    closureDate: closureDate,
                    finalClosureDate: moment(finalClosureDate),
                });
                console.log("form 2", form2.getFieldValue);
                setLoading(false);
            } catch (error) {
                throw error;
            }
